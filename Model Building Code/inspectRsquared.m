...
% since we have fixed an instruction (the middle one), we will have 5 options for 1st instruction and 5 options for 3rd instruction.
% And that's why we are having 25 sequences to worry about.
...
for j = 1:numel(t.oplist)
    ...
    for k = 1:numel(t.oplist)
        ...
        A = [ones(sum(idx), 1), zeros(sum(idx), 128)]; % here, size of A is 1000 x 129
...
fprintf('%s: Best choice is clock cycle %.0f', upper(t.oplist{i}), modelPt(i));
% For EOR instruction, It printed - EOR: Best choice is clock cycle 7
