...
laod('exampleDataCorr.mat');

% Mean-centre:
% mean(A, dim) - returns the mean along dimension dim.
% mean(A, 2) computes the mean of the elements in each row of A and returns a column vector.
traces = traces - repmat(mean(traces, 2), 1, size(traces, 2));

% Count the number of traces:
% traces - 1000 x 1002 - No. of traces = 1000, size of each trace = 1002.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Q: What is the point in drawing these plots?
% A: To provide some priliminary insights on the per-sequence measured traces intended for use in model building.
% There are 125 such sequences, we can do the same analysis for each of them.

% Above drawn six plots are:
% 1. Corr b/w traces and Hamming weights of inputs & outputs of the first instruction.
% 2. Corr b/w traces and Hamming weights of inputs & outputs of the second instruction.
% 3. Corr b/w traces and Hamming weights of inputs & outputs of the third instruction.

% 4. Corr b/w traces and Hamming Distances across the the first input bus.
% 5. Corr b/w traces and Hamming Distances across the the second input bus.
% 6. Corr b/w traces and Hamming Distances across the the output bus.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
