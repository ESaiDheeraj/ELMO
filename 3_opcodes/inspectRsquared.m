...
for ind = 1:numel(t.oplist) ^ 2
    ...
    plot([modelPt, modelPt], [getaxis(3), getaxis(4)], 'r:');
    title(titletxt{ind});
...
% subtitle(sprintf('%s: Best choice is clock cycle %.0f',upper(t.oplist{i}),modelPt(i)));
fprintf('%s: Best choice is clock cycle %.0f', upper(t.oplist{i}), modelPt);
